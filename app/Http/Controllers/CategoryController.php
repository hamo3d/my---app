...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
              ...
              // 1- sql
            // $data = DB::insert('insert into categories (name, info, active ) values (?, ?,?)', [$request->input('name'),$request->input('info'),$request->input('active')]);
...
//
    ...
    public function update(Request $request, string $id)
    {
        ...
        if(! $validatore->fails()){
            // $updateRowsCount = DB::update('update categories SET name = ?, info = ? , active = ? where id = ?  ', [$request->input('name'),$request->input('info'),$request->input('active'),$id]);
